...
class GraphMarkerView(context: Context?) : MarkerView(context, R.layout.view_marker) {
    ...
    override fun refreshContent(e: Entry, highlight: Highlight) {
        ...
        factionNameTextView.text = name
        influenceTextView.text = String.format(userLocale,
                "%.2f %%", influence * 100)
        stateTextView.text = state
        val date = LocalDateTime.ofInstant(updateDate, ZoneOffset.UTC)
        updateDateTextView.text = dateFormatter.format(date)
        super.refreshContent(e, highlight)
    }

    public fun setChartView(chart: LineChart) {
        chartView = chart
    }
